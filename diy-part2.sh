...
# Modify hostname
sed -i 's/OpenWrt/LeeSunWrt/g' package/base-files/files/bin/config_generate

# Modify the version number
sed -i 's/OpenWrt/LeeSunWrt build $(date "+%Y.%m.%d") @ OpenWrt/g' package/lean/default-settings/files/zzz-default-settings
...
[ -z $(grep "CONFIG_KERNEL_BUILD_USER=" .config) ] &&
    echo 'CONFIG_KERNEL_BUILD_USER="LeeSunWrt"' >>.config ||
    sed -i 's@\(CONFIG_KERNEL_BUILD_USER=\).*@\1$"LeeSunWrt"@' .config
