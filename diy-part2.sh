...
sed -i "23i uci set wireless.@wifi-device[0].disabled=1" package/lean/default-settings/files/zzz-default-settings
sed -i "24i uci set wireless.@wifi-device[1].disabled=1" package/lean/default-settings/files/zzz-default-settings

sed -i "25i sed -i '/option ra_management/d' /etc/config/dhcp" package/lean/default-settings/files/zzz-default-settings
sed -i "26i sed -i '/option dhcpv6/d' /etc/config/dhcp" package/lean/default-settings/files/zzz-default-settings
sed -i "27i sed -i '/option ra/d' /etc/config/dhcp" package/lean/default-settings/files/zzz-default-settings
sed -i "28i uci set dhcp.lan.ignore=1" package/lean/default-settings/files/zzz-default-settings
sed -i "29i uci commit dhcp" package/lean/default-settings/files/zzz-default-settings
...
sed -i 'chmod 777 /etc/smartdns/up.sh' package/lean/default-settings/files/zzz-default-settings
